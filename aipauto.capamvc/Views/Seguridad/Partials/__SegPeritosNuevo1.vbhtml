<fieldset class="form-area" data-bind="with: vi_segmanusu">
    ...
    <ul id="lista_err_segperitosnuevo1" style="display: none;">
...
<script type="text/javascript">
    function vmsegperitosnuevo1() {
        ...
        self.llenarcboidsegmanorg = function () {
            postajaxjsonbasico("/comun/ObtenerComboOrganizacion/", '{}', function (lista_comunmanempr) {
        ...
        self.llenarcboitipoperito = function () {
        ...
        self.agregar = function () {
            ...
            if (self.vi_segmanusu.errors().length > 0) {
                generar_noty_modal($('#lista_err_segperitosnuevo1').html(), "warning");
                return false;
            }

            

            postajaxjsonbasico("/Mantenimiento/AgregarUsuarioPerito/?xcodimmandisp=" + self.xcodimmandisp() + "&itipoperito=" + self.itipoperito(), ko.toJSON(self.vi_segmanusu), function (respopera) {
                if (respopera.irespuesta == 1) {
                    generar_noty_auto("El perito se ha creado con éxito.", "success");
                    self.limpiar();
