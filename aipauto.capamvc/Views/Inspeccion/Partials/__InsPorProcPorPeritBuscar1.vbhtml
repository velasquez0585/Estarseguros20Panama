...
<script type="text/javascript">
    function vmInsPorProcPorPeritBuscar1() {
        var self = this;
        ...
        self.buscar = function () {
            if (self.vi_aipinsenc.idsegmanusuperit() > 0) {
                postajaxjsonbasico("/Inspeccion/ObtenerInspeccionesPorProcesarPorPerito/", ko.toJSON(self), function (respvi_aipinsencpaglist) {
                    ko.mapping.fromJS(respvi_aipinsencpaglist, {}, self);
                })
            }
            else {
                generar_noty_modal("Ha ocurrido un error con el perito seleccionado", "warning");
            }
        }


        self.iniciar = function (idsegmanusuperit) {            
            self.vi_aipinsenc.idsegmanusuperit(idsegmanusuperit);
            self.buscar();
