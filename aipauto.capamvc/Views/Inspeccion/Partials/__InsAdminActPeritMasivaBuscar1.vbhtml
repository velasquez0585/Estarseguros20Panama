<fieldset class="form-area" data-bind="with: vi_aipinsenc">
    ...
    <div class="form-column">
        <div class="form-field-header">
            ...
            <div class="form-field-content">      
                <select data-bind="options: $root.cboitipoperit, optionsText: 'xvalordescripcion', optionsValue: 'xvalorcodigo', value: $root.itipoperit, event: { change: $root.evento_itipoperit }"></select>
            </div>
        </div>
    </div>
    ...
    <div class="form-column">
        <div class="form-field-header">
            <label>Perito</label>
            <div class="form-field-content">                
                <select data-bind="options: $root.cbousuarios, optionsText: 'xnombrecompleto', optionsValue: 'idsegmanusu', value: idsegmanusuperit"></select>
            </div>
        </div>
    </div>
...
<fieldset class="form-area">
    ...
    <div class="row">
        ...
        <div class=""  data-bind="visible: lista_vi_aipinsenc().length > 0" >
            ...
            <div class="form-field-header">
                <label>Nuevo Perito:</label>
                <div class="form-field-content">                
                    <select data-bind="options: $root.cbousuarios, optionsText: 'xnombrecompleto', optionsValue: 'idsegmanusu', value: $root.idsegmanusuperit"></select>
...
<script type="text/javascript">
    function vminsadminactperitmasivabuscar1() {
        var self = this;
        ...
        self.cboitipoperit = ko.observableArray();

        self.itipoaccion = ko.observable("PERITO");
        self.disabled_accion = ko.observable(true);
        self.disabled_sucursal = ko.observable(true);
        self.itipoperit = ko.observable("");
        ...
        self.cargar = function () {
            ...
            self.itipoaccion("PERITO");
            self.llenarcboitipoperit();
        ...
        self.buscar = function (tipo) {
            ...
            if (self.valido()) {
                ...
                if (self.vi_aipinsenc.idsegmanusuperit() == 0) {
                    generar_noty_modal("Debe seleccionar el 'Perito'", "warning");
                    return false;
                }


                self.lista_vi_aipinsenc("");
                postajaxjsonbasico("/Inspeccion/ObtenerInspeccionesPorTipo/?itipoaccion=PERITO", ko.toJSON(self), function (respvi_aipinsencpaglist) {
        ...
        self.llenarcboitipoperit = function (itipoaccion) {
            var accion = [
                { 'xvalorcodigo': '', 'xvalordescripcion': 'SELECCIONE...' },
                { 'xvalorcodigo': 'EXTERNO', 'xvalordescripcion': 'EXTERNO' },
                { 'xvalorcodigo': 'INTERNO', 'xvalordescripcion': 'INTERNO' }
            ];

            self.cboitipoperit(accion);                       
        }
        ...
        self.llenarcbousuarios = function () {
            ...
            if (self.idsegmanorgperit() > 0 && self.itipoperit() != "")
            {
                postajaxjsonbasico("/Seguridad/ObtenerComboPorAccionOrgTipoPeritYTipoUsuario/?idsegmanorg= " + self.idsegmanorgperit() + "&itipoaccion=PERITO&itipoperit=" + self.itipoperit(), '{}', function (lista_vi_segusudetperf2) {
        ...
        self.actualizar = function () {
            ...
            if (self.idsegmanusuperit() == self.vi_aipinsenc.idsegmanusuperit()) {
                generar_noty_modal("Debe seleccionar un Perito distinto para la actualización.", "warning")
                return false;
            }

            if (self.idsegmanusuperit() == 0) {
                generar_noty_modal("Debe seleccionar el Perito de Destino para la actualización.", "warning")
                return false;
            }

            postajaxjsonbasico("/Inspeccion/ModificarPeritoMasivo/?idsegmanusuperit=" + self.idsegmanusuperit(), ko.toJSON(self.lista_vi_aipinsenc()), function (respopera) {
            ...
            if (self.lista_vi_aipinsenc().length > 0)
            {
                ...
                var botones = [
                {
                    addClass: 'btn btn-primary', text: 'Si', onClick: function ($noty) {
                        ...
                        postajaxjsonbasico("/Inspeccion/ModificarPeritoInspeccionTodo/?idsegmanusuperit=" + self.idsegmanusuperit(), ko.toJSON(self.lista_vi_aipinsenc()), function (respopera) {
                ...
                generar_noty_confirmacion("¿Está seguro que desea actualizar todas las inspecciones al nuevo perito?", botones);
            }
        }
        ...
        this.evento_itipoperit = function (obj, event) {
            if (event.originalEvent) {
                //si el usuario desencadeno el evento
                self.lista_vi_aipinsenc("");
                self.pagvi_aipinsenc.npagactual(1);
    ...
    vminsadminactperitmasivabuscar1.prototype.toJSON = function () {
